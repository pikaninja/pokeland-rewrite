...
class Slash(commands.Cog):
    ...
    DISCORD_TYPES = [str, int, bool, discord.User, discord.Member, discord.TextChannel, discord.Role]
    ...
    async def parse_options(self, command):
        if isinstance(command, commands.Group):
            return [
                {
                    "name": command.name,
                    "description": self.format_string(command.help or "no decription", 100),
                    "type": 1,
    ...
    async def build_slash_commands(self):
        ...
        cmds = [
            {
                "name": command.name,
                "description": self.format_string(command.help or "no description", 100),
                "options": await self.parse_options(command)
            }
            for command in self.bot.commands if command.qualified_name != "jishaku"
        ]
    ...
    async def pokemon(self, interaction):
        ...
        for option in options:
            if option['name'] == "page":
                ...
                setattr(flags, option['name'], option['value'])

        print(flags.name)

        await self.bot.get_command("pokemon")(SlashContext(interaction.user, interaction.channel, interaction.message, interaction), page, flags = flags)
    ...
    async def on_interaction(self, interaction: discord.Interaction):
        ...
        params = " ".join(f"{name}: {str(option['value'])}" for name, option in options.items())
        content = f"/{command} {params}"
        message = FalseMessage(content, discord.utils.utcnow())
        ctx = SlashContext(self.bot, interaction.user, interaction.channel, interaction.guild, message, interaction)
        params = []
        kwargs = {}
        ...
        async def fallback():
            ...
            if interaction.response.is_done():
                return
            await interaction.response.send_message(f"You invoked the command {command}.")
...
#            return
#
#        async def just_in_case():
#            await asyncio.sleep(3)
#            if not interaction.response.is_done():
#                await interaction.response.defer()
#        self.bot.loop.create_task(just_in_case())
