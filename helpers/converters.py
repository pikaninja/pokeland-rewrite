...
class PokemonConverter(commands.Converter):
    async def convert(self, ctx, argument):
        if not argument:
            argument = await ctx.bot.connection.fetchval("SELECT selected FROM users WHERE id = $1", ctx.author.id)
        ...
        async with ctx.bot.connection.acquire() as conn:
            async with conn.transaction():
                ...
                pokemon = await ctx.bot.db.get_pokemon_by_idx(ctx.author, argument, connection=conn)
